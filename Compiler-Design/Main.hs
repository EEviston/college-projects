...
main = do
  ...
  if length args == 2 then do
    ...
    let gConstants = B.split ':' assemblerFile'

    let gConstantsElem = (gConstants !! 0)

    let gConsList = B.split '\n' gConstantsElem
    let newgConsList = init gConsList

    let singletonVal = B.singleton '\n'

    let finalList = B.intercalate singletonVal newgConsList

    let assemblerFile = finalList `B.append` "\n" `B.append` "Call 0 Main\nJmp $END\n" `B.append` assemblerFile'
                        `B.append` "\n$END: Halt\n"
