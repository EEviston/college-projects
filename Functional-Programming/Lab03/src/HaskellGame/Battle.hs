...
instance Combatant Player where -- Task 2 : need to implement the five functions below
  health (Player hp _ _ _ _) = hp 

  attack (Player _ _ stats skill _) = snd(stats List.!! 0) * snd(skill List.!! 0)

  defense (Player _ _ stats _ _) = snd(stats List.!! 1)

  level (Player _ _ stats _ _) = floor(log(fromIntegral(snd(stats List.!! 0) + snd(stats List.!! 1))))

  damage (Player hp a b c d) dmg
    | dmg < 0 = (Player hp a b c d)
    | otherwise = (Player (hp-dmg) a b c d)
