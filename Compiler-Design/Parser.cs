...
/*--------------------------------------------------------------------------*/
  ...
  public void Write (string filename) {
	...
	void Factor(out TastierType type) {
		...
		if (la.kind == 1) {
			...
			if (sym == null) {
			 ...
			 if ((TastierKind)sym.Item2 == TastierKind.Var || (TastierKind)sym.Item2 == TastierKind.Constant) {
			   if (sym.Item4 == 0) {
			       if (isExternal) {
	...
	void ConstDecl(bool external) {
		string name; TastierType type; Scope currentScope = openScopes.Peek(); TastierType altType;
		
		Expect(31);
		Type(out type);
		Ident(out name);
		if (external) {
		 Console.WriteLine("Name (external variable from Test.TAS) = " + name + ", Type = " + type + ", Address (Stack Frame Index) = 0");
		 externalDeclarations.Push(new Symbol(name, (int)TastierKind.Constant, (int)type, 0, 0));
		} else {
		 if ((openScopes.Count-1) == 0) {
		   Console.WriteLine("This is a global constant.");
		   Console.WriteLine("Name = " + name + ", Type = " + type + ", Stack Frame = " + (openScopes.Count-1) +
		     ", Address (Stack Frame Index) = " + currentScope.Count(s => (s.Item2 == (int)TastierKind.Constant) || (s.Item2 == (int)TastierKind.Var)));
		   currentScope.Push(new Symbol(name, (int)TastierKind.Constant, (int)type, openScopes.Count-1, currentScope.Count(s => (s.Item2 == (int)TastierKind.Constant) || (s.Item2 == (int)TastierKind.Var))));
		 } else {
		   Console.WriteLine("The is a local constant.");
		   Console.WriteLine("Name = " + name + ", Type = " + type + ", Stack Frame = " + (openScopes.Count-1) +
		     ", Address (Stack Frame Index) = " + currentScope.Count(s => (s.Item2 == (int)TastierKind.Constant) || (s.Item2 == (int)TastierKind.Var)));
		   currentScope.Push(new Symbol(name, (int)TastierKind.Constant, (int)type, openScopes.Count-1, currentScope.Count(s => (s.Item2 == (int)TastierKind.Constant) || (s.Item2 == (int)TastierKind.Var))));
		 }
		...
		if ((openScopes.Count-1) == 0) {
		 if (external) {
		   ...
		   program.Add(new Instruction("", "StoG " + (currentScope.Count(s => (s.Item2 == ((int)TastierKind.Constant) || (s.Item2 == (int)TastierKind.Var)))+2)));
		 }
		}
		else {
		 int lexicalLevelDifference = Math.Abs(openScopes.Count - (openScopes.Count-1))-1;
		 program.Add(new Instruction("", "Sto " + lexicalLevelDifference + " " + (currentScope.Count(s => (s.Item2 == (int)TastierKind.Constant) || (s.Item2 == (int)TastierKind.Var)))));
		}
	...
	void VarDecl(bool external) {
		...
		if (external) {
		 ...
		 if ((openScopes.Count-1) == 0) {
		   ...
		   Console.WriteLine("Name = " + name + ", Type = " + type + ", Stack Frame = " + (openScopes.Count-1) +
		     ", Address (Stack Frame Index) = " + currentScope.Count(s => (s.Item2 == (int)TastierKind.Var) || (s.Item2 == (int)TastierKind.Constant)));
		   currentScope.Push(new Symbol(name, (int)TastierKind.Var, (int)type, openScopes.Count-1, currentScope.Count(s => (s.Item2 == (int)TastierKind.Var) || (s.Item2 == (int)TastierKind.Constant))));
		 } else {
		   Console.WriteLine("The is a local variable.");
		   Console.WriteLine("Name = " + name + ", Type = " + type + ", Stack Frame = " + (openScopes.Count-1) +
		     ", Address (Stack Frame Index) = " + currentScope.Count(s => (s.Item2 == (int)TastierKind.Var) || (s.Item2 == (int)TastierKind.Constant)));
		   currentScope.Push(new Symbol(name, (int)TastierKind.Var, (int)type, openScopes.Count-1, currentScope.Count(s => (s.Item2 == (int)TastierKind.Var) || (s.Item2 == (int)TastierKind.Constant))));
		 }
		...
		while (la.kind == 27) {
			...
			if (external) {                                   
			 ...
			 if ((openScopes.Count-1) == 0) {
			   ...
			   Console.WriteLine("Name = " + name + ", Type = " + type + ", Stack Frame = " + (openScopes.Count-1) +
			     ", Address (Stack Frame Index) = " + currentScope.Count(s => s.Item2 == (int)TastierKind.Var));
			   currentScope.Push(new Symbol(name, (int)TastierKind.Var, (int)type, openScopes.Count-1, currentScope.Count(s => (s.Item2 == (int)TastierKind.Var) || (s.Item2 == (int)TastierKind.Constant))));
			 }
			}
	...
	void Tastier() {
		...
		foreach (Symbol s in externalDeclarations) {
		 if (s.Item2 == (int)TastierKind.Var) {
		   ...
		   header.Add(new Instruction("", ".external const " + s.Item1));
		 } else {
	...
	void ExternDecl() {
		...
		if (la.kind == 29 || la.kind == 30) {
			...
			Ident(out name);
			Expect(21);
			Console.WriteLine("Name (external process from Test.TAS) = " + name);
	...
	static readonly bool[,] set = {
		...
		{x,T,x,x, x,x,x,x, x,T,x,x, T,x,x,x, x,x,x,x, x,x,T,x, T,T,T,x, x,T,T,T, x,x,x,x},
		{x,T,x,x, x,x,x,x, x,x,x,x, T,x,x,x, x,x,x,x, x,x,T,x, T,T,T,x, x,x,x,x, x,x,x,x},
		{x,T,x,x, x,x,x,x, x,x,x,x, T,x,x,x, x,x,x,x, x,x,T,x, T,T,T,x, x,T,T,T, x,x,x,x},
		{x,x,x,x, x,x,x,x, x,T,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,T,T,T, T,x,x,x}
...
public class Errors {
	...
	public virtual void SynErr (int line, int col, int n) {
		...
		switch (n) {
			...
			case 31: s = "\"const\" expected"; break;
